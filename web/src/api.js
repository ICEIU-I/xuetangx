...
export const api = {
  ...
  run: (targets, courseUrl, concurrency = 3) => j('/api/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ targets, courseUrl, concurrency }) }),
  ...
  discussionRun: (courseUrl, concurrency = 3) => j('/api/discussion/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ courseUrl, concurrency }) }),
