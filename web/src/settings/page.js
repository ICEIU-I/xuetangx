...
export function mountSettings(host, workspace) {
  ...
  host.innerHTML = '<header class="page-heading"><h1>账号设置</h1></header><div class="settings-stack"><div class="platform-settings"></div><div class="password-settings"></div><div class="token-settings"></div></div>';
  ...
  life.add(mountPassword(host.querySelector('.password-settings'))); life.add(mountTokens(host.querySelector('.token-settings'))); life.add(workspace.subscribe(draw)); life.add(() => disposeConnect()); draw(); return life.dispose;
}
