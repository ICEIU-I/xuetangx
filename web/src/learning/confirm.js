...
export function mountConfirm(host, workspace) {
  ...
  function draw() {
    ...
    render(host, `<section class="surface course-confirm"><header class="course-header"><span class="section-index">02</span><div><span class="muted">学堂在线</span><h2>${e(state.courses[0]?.title || '正在读取课程…')}</h2></div><span class="status-pill">题库已匹配</span></header>${scoreMarkup(state)}<div class="course-features">${modules.map((m,i) => `<div class="course-feature" data-module="${m.kind}"><span>0${i+1}</span><strong>${m.title}</strong></div>`).join('')}</div><p class="enroll-note">未加入课程时，自动免费加入。</p><div class="task-actions"><button id="start-task" class="primary"${disabled(state.starting || !state.courses.length)}>${state.starting ? '正在检查课程并准备任务…' : state.pendingStart ? '重新核对任务状态' : '开始任务'}</button></div>${feedback(state.error)}</section>`);
  }
