...
export function mountLearn(host, workspace) {
  ...
  host.innerHTML = '<header class="page-heading"><h1>课程任务</h1><span class="account-chip"></span></header><ol class="wizard-steps" aria-label="课程任务步骤"></ol><div class="step-content"></div>';
  ...
  function update() {
    ...
    const next = state.loading ? 'loading' : state.error && !state.courses.length ? `error:${state.error}` : `${step}:${job?.id || ''}`;
