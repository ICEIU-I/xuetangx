...
export function canShowScore(state, job) {
  return state.session.connected && (!job || (Number(job.primaryId) === primaryId(state.session) && Number(job.course.classroomId) === Number(state.courses[0]?.classroomId)));
}
export function scoreMarkup(state, job) {
  ...
  const available = score.primaryId === account && score.available;
