...
export function mountRunner(host, workspace, course, kind) {
  ...
  life.add(delegate(content,'click','#tool-start',async () => {
    ...
    try { const result = await workspace.start(course.url,concurrency,{ modules:[kind], ...(kind === 'collector' ? { submitUnanswered:submit } : {}), ...(kind === 'homework' && selected.length ? { targets:selected } : {}) }); if (life.alive && result) navigate(`/tasks/${result.id}`); else error = workspace.state.error; }
