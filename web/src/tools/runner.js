...
export function mountRunner(host, workspace, course, kind) {
  ...
  const search = listSearch(content, life, 'tool-item-search', '搜索项目名称', () => draw(), () => busy);
  const visibleSections = () => (scan?.sections || []).filter(s => matchesSearch(search.query,s.name));
  const visibleItems = () => (scan?.[kind === 'discussion' ? 'discussions' : kind+'s'] || []).filter(i => matchesSearch(search.query,i.title));
  ...
  function draw() {
    ...
    const job = current();
    render(content, `<header class="section-heading"><h2>${title}</h2></header>${running() ? `<div class="feedback waiting"><span>已有${statusNames[job.status]}的任务</span><a data-route href="/tasks/${encodeURIComponent(job.id)}">查看任务 →</a></div>` : ''}<div class="tool-options">${kind === 'collector' ? `<label class="checkbox-label"><input id="collector-submit" type="checkbox"${submit ? ' checked' : ''}${disabled(locked())}>使用采集账号补齐未公开答案</label>` : ''}</div>${kind === 'collector' ? `<p class="muted">${submit ? '将使用采集账号提交未做题，可能消耗作答机会。' : '仅读取公开答案。'}</p>` : ''}<div class="row"><button id="tool-start" class="primary"${disabled(locked())}>${busy ? '处理中…' : `开始${title}任务`}</button>${kind !== 'collector' ? `<button id="tool-scan"${disabled(locked())}>${kind === 'homework' ? '选择作业' : '查看课程项目'}</button>` : ''}</div>${workspace.state.pendingStart ? '<p class="feedback waiting">正在核对已请求的任务，请稍后查看任务记录。</p>' : ''}${scan ? `<div class="scan-results">${search.markup()}${kind === 'homework' ? `<p class="muted">不勾选时处理全部未完成作业。</p>${visibleSections().map(section => `<label class="checkbox-label"><input data-section type="checkbox" value="${e(section.name)}"${selected.includes(section.name) ? ' checked' : ''}><span>${e(section.name)}</span><span class="muted">已做 ${section.done}/${section.total} · 答对 ${section.right}</span></label>`).join('')}` : `<p>共 ${scan.total} 项，已完成 ${scan.completed} 项</p>${visibleItems().map(item => `<div class="scan-item"><span>${e(item.title)}</span><span>${item.completed ? '已完成' : item.locked ? '尚未开放' : '未完成'}</span></div>`).join('')}`}${!(kind === 'homework' ? visibleSections() : visibleItems()).length ? '<p class="empty-state">没有匹配的项目</p>' : ''}</div>` : ''}${feedback(error)}`);
  }
