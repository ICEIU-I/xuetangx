...
export function mountTools(host, workspace, route) {
  const life = lifetime(); const tools = [...modules, {kind:'collector',title:'答案采集'}];
  ...
  const search = listSearch(controls, life, 'tool-course-search', '搜索课程名称或班级 ID', () => { const filtered = visibleCourses(); if (!filtered.some(c => c.url === selected)) { selected = filtered[0]?.url || ''; mountChild(); } draw(); }, () => loading);
  const visibleCourses = () => courses.filter(c => matchesSearch(search.query, c.title, c.classroomId));
  function draw() { render(controls, !workspace.state.session.connected ? '<div class="surface empty-state"><p>请先连接学堂在线</p><a data-route class="button primary" href="/learn">连接账号</a></div>' : `<nav class="tool-navigation" aria-label="工具导航">${tools.map(t => `<a data-route href="/tools?tool=${t.kind}" class="${kind === t.kind ? 'selected' : ''}">${t.title}</a>`).join('')}</nav>${search.markup()}<div class="surface tool-course"><label for="tool-course">课程</label><select id="tool-course"${disabled(loading)}><option value="" disabled${!selected ? ' selected' : ''}>${loading ? '正在读取课程…' : '请选择课程'}</option>${visibleCourses().map(c => `<option value="${e(c.url)}"${selected === c.url ? ' selected' : ''}>${e(c.title)}</option>`).join('')}</select><button id="tool-refresh"${disabled(loading)}>刷新</button>${feedback(error)}${!loading && !visibleCourses().length ? '<p class="muted">没有匹配的课程</p>' : ''}</div>`); }
  function mountChild() { disposeChild(); content.innerHTML = ''; if (!workspace.state.session.connected) return; const course = courses.find(c => c.url === selected); disposeChild = mountRunner(content,workspace,course,kind); }
  async function load() { const token = ++revision; courses = []; selected = ''; error = ''; loading = true; draw(); mountChild(); try { const result = await api.videoCourses(); if (!life.alive || token !== revision) return; courses = result.courses || []; selected = visibleCourses()[0]?.url || '';  } catch (err) { error = err.message; } finally { if (life.alive && token === revision) { loading = false; draw(); mountChild(); } } }
