...
async function start() {
  ...
  try { await api.articleRun(courseUrl.value); } catch (e) { error.value = e.message; }
  ...
  <section class="panel article-panel" aria-labelledby="article-title">
    ...
    <div v-if="session.connected && task.status !== 'idle'" class="task" aria-live="polite">
      ...
      <div v-if="task.results?.length" class="results">
        <div v-for="item in task.results" :key="item.leafId" class="result-row"><span>{{ item.title }}</span><span :class="item.status === 'failed' ? 'error' : 'dim'">{{ item.error || (item.status === 'skipped' ? '已看完，跳过' : '后端确认已看完') }}</span></div>
      </div>
    </div>
