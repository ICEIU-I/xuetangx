...
async function main() {
  ...
  if (urls.length !== 1 || args.some(arg => arg.startsWith('--') && arg !== '--complete' && !arg.startsWith('--concurrency='))) throw new Error('用法：npm run discussion -- <课程或讨论题链接> [--complete]；默认只查询');
  ...
  try {
    ...
    const result = args.includes('--complete') ? await discussions.completeCourse({ courseUrl: urls[0], concurrency }, COOKIE, callbacks) : await discussions.scanCourse(urls[0], COOKIE, callbacks);
