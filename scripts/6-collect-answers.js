...
async function main() {
  ...
  if (urls.length !== 1 || args.some(arg => arg.startsWith('--') && arg !== '--submit-unanswered')) throw new Error('用法：npm run collect-answers -- <课程学习页链接> [--submit-unanswered]');
  ...
  try {
    ...
    const result = await bank.collect({ courseUrl: urls[0], submitUnanswered }, COOKIE,
