...
- 📄 **图文标记看完**：选择一门课程，一键串行标记全部图文，跳过已完成项并逐项回查。
...
在「图文进度」选择课程，点击「一键标记本课程全部图文」。功能仅遍历该课程的图文单元（`leaf_type=3`），包含 PDF 附件所在的图文学习单元；已完成项自动跳过，未完成项按顺序标记，随后读取详情确认 `finish=true`。也可先点击「查看图文进度」。
...
任务串行执行，可停止；HTTP 429 等待后重试，其他不确定的发布错误不自动重发。本地 `data/discussion-state/` 按课程和账号保存发布记录，以便中断后核对并避免重复评论。若发布成功但平台尚未计入进度，会显示未完成项，保留记录供后续回查。

## 课程答案库（JSON Demo）

连接登录态后，在「课程答案库」中选择一门课程，点击「获取本课程已公开答案」。需要获取未做题的答案时，勾选「提交未做题后采集答案」，再点击「提交并采集本课程答案」。这一模式会提交测试选项或占位内容，消耗作答机会并可能影响成绩；已作答题不会重复提交。采集始终串行，支持停止。
...
npm run collect-answers -- '<课程学习页URL>' --submit-unanswered  # 串行提交未做题并采集标准答案
npm run article -- '<课程或图文学习页URL>'           # 查询该课程全部图文进度
npm run article -- '<课程或图文学习页URL>' --complete  # 一键串行标记该课程全部图文
