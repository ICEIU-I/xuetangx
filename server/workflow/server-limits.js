...
function createServerLimits({ now = Date.now } = {}) {
  ...
  function observe(userId, response) {
    ...
    cooldowns.set(userId, Math.max(cooldowns.get(userId) || 0, time + delay));
    return snapshot(userId);
  }
  function snapshot(userId) {
    const readyAt = cooldowns.get(userId) || 0, blocked = readyAt > now();
    if (!blocked) cooldowns.delete(userId);
    return { userId, source: 'server', blocked, readyAt: blocked ? readyAt : null };
  }
