...
func (r *Repository) Recover(ctx context.Context) error {
	return r.DB.Tx(ctx, func(tx pgx.Tx) error {
		...
		rows, err := tx.Query(ctx, `SELECT j.id,j.owner_id FROM jobs j
			...
			((m.message LIKE '%已冷却重试 5 次%' OR m.message='平台拒绝访问（HTTP 403）') OR (m.kind IN ('video','article') AND EXISTS(SELECT 1 FROM job_items i WHERE i.job_id=j.id AND i.kind=m.kind AND i.error IN ('上次操作结果不确定，未重复发送','进度回查缺少有效完成状态'))))) FOR UPDATE OF j`)
		if err != nil {
			return err
		}
		...
		for _, c := range all {
			if _, err = tx.Exec(ctx, `UPDATE job_modules m SET status='queued',generation=generation+1,message='已修复自动重试，正在回查未完成项'
				...
				OR (kind IN ('video','article') AND EXISTS(SELECT 1 FROM job_items i WHERE i.job_id=m.job_id AND i.kind=m.kind AND i.error IN ('上次操作结果不确定，未重复发送','进度回查缺少有效完成状态'))))`, c.id); err != nil {
				return err
			}
